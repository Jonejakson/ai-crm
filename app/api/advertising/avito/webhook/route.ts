...
// Обработать заявку из Авито
async function processAvitoApplication(
  application: any,
  integration: any
  ...
  const itemId = application.itemId || application.adId || application.ad_id || null
  const applicationId = application.id || application.applicationId || null
    ...
    if (integration.autoCreateDeal && integration.defaultPipelineId) {
      ...
      if (pipeline) {
        ...
        const dealTitle = comment 
          ? `Заявка с Авито: ${comment.substring(0, 50)}`
          : `Заявка с Авито: ${name}`
